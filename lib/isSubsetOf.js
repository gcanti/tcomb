...
var isTypedFunction = require('./isTypedFunction');
...
function isSubsetOf(subset, type) {
  ...
  } else if (isFunc(subset) || isTypedFunction(subset)) {
    dispatch = func;
...
function func(func, type) {
  if (type === tFunction) {
    return true;
  } else if (isFunc(type) || isTypedFunction(type)) {
    var funcMeta = isFunc(func) ? func.meta : func.instrumentation;
    var typeMeta = isFunc(type) ? type.meta : type.instrumentation;

    return isSubsetOf(funcMeta.codomain, typeMeta.codomain) && funcMeta.domain.length === typeMeta.domain.length &&
      funcMeta.domain.every(function (func, i) {
        return isSubsetOf(func, typeMeta.domain[i]);
      });
  } else {
    return false;
  }
