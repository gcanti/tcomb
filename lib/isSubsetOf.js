...
function isSubsetOfImpl(A, B) {
  ...
  if (kindB === 'intersection') {
    if (B.meta.types.every(isSupersetOfA)) {
      return true;
    }
  }
  ...
  // Let A be a refinement then A <= B if decompose(A) <= decompose(B)
    ...
    var dA = decompose(A);
    var dB = decompose(B);
    return isSubPredicatesOf(dA.predicates, dB.predicates) && recurse(dA.unrefinedType, dB.unrefinedType);
  }
  ...
  // Let A be a function then then A <= B if one of the following holds:
    ...
    return ( kindB === 'func' ) && recurse(A.meta.codomain, B.meta.codomain) && isSubsetsOf(A.meta.domain, B.meta.domain);
  }
...
function recurse(A, B) {
  ...
  hit = { A: A, B: B, isSubset: true };
  index.push(hit);
  hit.isSubset = isSubsetOfImpl(A, B);
  return hit.isSubset;
}
